/**
 * SWORD Verse Key System
 * 
 * Handles verse addressing and index calculations for SWORD modules.
 * Based on the KJV versification system by default.
 */
...
/**
 * Verse counts per chapter for KJV versification
 * This is the default versification used by most SWORD modules
 */
const KJV_VERSE_COUNTS: Record<string, number[]> = {
  ...
  'Prov': [33,22,35,27,23,35,27,36,18,32,31,28,25,35,33,33,28,24,29,30,31],
...
/**
 * Calculate the absolute verse index for a verse reference
 * Used to look up verses in SWORD module index files
 */
export function getVerseIndex(ref: VerseRef): number {
  let index = 0;
  ...
  for (const book of BIBLE_BOOKS) {
    ...
    const verseCounts = KJV_VERSE_COUNTS[book.id];
    if (verseCounts) {
      index += verseCounts.reduce((a, b) => a + b, 0);
    }
  }
  
  // Add verses from previous chapters in this book
  const verseCounts = KJV_VERSE_COUNTS[ref.book];
  if (verseCounts) {
    for (let c = 0; c < ref.chapter - 1 && c < verseCounts.length; c++) {
      index += verseCounts[c];
    }
  }
  
  // Add the verse number (1-based to 0-based)
  index += ref.verse - 1;
...
/**
 * Calculate the chapter index (for chapter-block compressed modules)
 */
 ...
 */
export function parseVerseRef(refString: string): VerseRef | null {
...
/**
 * Format a verse reference to OSIS format
 */
...
/**
 * Get all verse refs for a chapter
 */
export function getChapterVerseRefs(book: string, chapter: number): VerseRef[] {
...
/**
 * Total verse count in the Bible (KJV versification)
 */
export const TOTAL_VERSE_COUNT = 31102;

/**
 * Total chapter count in the Bible
 */
export const TOTAL_CHAPTER_COUNT = 1189;
